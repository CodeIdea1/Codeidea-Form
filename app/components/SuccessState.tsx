...
export default function SuccessState({ name, tr, lang }: { name: string; tr: Translations; lang: Lang }) {
  ...
  return (
    <div style={{ minHeight: "100svh", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: mirror ? "flex-end" : "flex-start", padding: "clamp(2rem,8vw,7rem) clamp(1.5rem,8vw,7rem)", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: "700px", textAlign: mirror ? "right" : "left" }}>
        <div style={{ marginBottom: "clamp(2rem,5vw,3.5rem)", animation: "fadeUp 0.8s cubic-bezier(0.16,1,0.3,1) 0.1s both", display: "flex", justifyContent: mirror ? "flex-end" : "flex-start" }}>
        ...
        <p style={{ fontFamily: dFont, fontSize: isAr ? "clamp(1rem,2.2vw,1.3rem)" : "clamp(1rem,2.1vw,1.3rem)", color: "var(--fg-dim)", fontWeight: isAr ? 400 : 300, lineHeight: isAr ? 1.9 : 1.7, maxWidth: "460px", marginBottom: "clamp(2.5rem,6vw,5rem)", animation: "fadeUp 0.9s cubic-bezier(0.16,1,0.3,1) 0.4s both", marginLeft: mirror ? "auto" : 0 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", animation: "fadeUp 0.9s cubic-bezier(0.16,1,0.3,1) 0.6s both" }}>
          {tr.miniJourney.map((item, i) => (
            <div key={item.status} style={{ display: "flex", alignItems: "center", gap: "clamp(1rem,3vw,2rem)", padding: "clamp(0.6rem,1.2vw,0.85rem) 0", borderBottom: i < tr.miniJourney.length - 1 ? "1px solid var(--line)" : "none", opacity: step > i ? 1 : 0.2, transition: "opacity 0.5s ease", flexDirection: mirror ? "row-reverse" : "row" }}>
              <span style={{ fontFamily: mFont, fontSize: isAr ? "0.85rem" : "0.7rem", letterSpacing: isAr ? "0" : "0.15em", color: item.done ? "var(--accent)" : "var(--fg-muted)", textTransform: isAr ? "none" : "uppercase", minWidth: isAr ? "4rem" : "3.5rem", textAlign: mirror ? "right" : "left" }}>{item.status}</span>
