    ...
    <section className={s.section}>
      ...
      <div className={s.grid}>
        ...
        <div style={{ textAlign: isAr ? "right" : "left", order: isAr ? 2 : 1 }}>
          ...
          <p style={{ fontFamily: dFont, fontSize: isAr ? "clamp(0.95rem,1.8vw,1.1rem)" : "clamp(0.95rem,1.6vw,1.05rem)", color: "var(--fg-muted)", fontWeight: isAr ? 400 : 300, lineHeight: isAr ? 1.9 : 1.7, maxWidth: "360px", marginLeft: isAr ? "auto" : 0 }}>
            {tr.whatFootnote}
          </p>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", order: isAr ? 1 : 2 }}>
          {tr.disciplines.map((d, i) => (
            ...
            >
              ...
              <span style={{ fontFamily: "var(--font-geist-mono)", fontSize: "0.85rem", color: "var(--fg-muted)", opacity: 0.4, transform: isAr ? "scaleX(-1)" : "none" }}>→</span>
            </div>
          ))}
        </div>
      </div>
