  ...
  return (
    <section style={{ width: "100%", height: "100%", minHeight: "100svh", display: "flex", flexDirection: "column", justifyContent: "center", padding: "clamp(4rem,8vw,6rem) clamp(1.5rem,8vw,7rem)", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ marginBottom: "clamp(1.5rem,4vw,3rem)", display: "flex", alignItems: "center", gap: "1rem", flexDirection: isAr ? "row-reverse" : "row" }}>
        <span style={{ fontFamily: mFont, fontSize: isAr ? "0.85rem" : "0.6rem", letterSpacing: isAr ? "0" : "0.25em", color: "var(--fg-muted)", textTransform: isAr ? "none" : "uppercase" }}>{tr.sectionLabel03}</span>
        <div style={{ flex: 1, height: "1px", background: "var(--line)", maxWidth: "80px" }} />
      </div>

      <h2 style={{ fontFamily: hFont, fontSize: "clamp(1.6rem,4vw,3rem)", fontWeight: 700, letterSpacing: isAr ? "-0.01em" : "-0.025em", lineHeight: isAr ? 1.3 : 1.1, color: "var(--fg)", marginBottom: "clamp(2rem,5vw,4rem)", maxWidth: "600px", textAlign: isAr ? "right" : "left", marginLeft: isAr ? "auto" : 0 }}>
          ...
          {tr.steps.map((step, i) => (
            <div key={step.num} style={{ position: "relative", paddingTop: "clamp(1.5rem,3vw,2.5rem)", textAlign: isAr ? "right" : "left" }}>
              ...
              <div style={{ display: "flex", alignItems: "baseline", gap: "0.5rem", marginBottom: "0.5rem", flexDirection: isAr ? "row-reverse" : "row" }}>
                <span style={{ fontFamily: mFont, fontSize: isAr ? "0.75rem" : "0.55rem", color: "var(--accent-dim)", letterSpacing: isAr ? "0" : "0.15em" }}>{step.num}</span>
                <span style={{ fontFamily: mFont, fontSize: isAr ? "0.75rem" : "0.55rem", color: "var(--fg-muted)", letterSpacing: isAr ? "0" : "0.15em", textTransform: isAr ? "none" : "uppercase" }}>{step.title}</span>
              </div>

              <h3 style={{ fontFamily: hFont, fontSize: "clamp(0.95rem,2vw,1.3rem)", fontWeight: 700, color: "var(--fg)", marginBottom: "0.5rem", lineHeight: isAr ? 1.4 : 1.2, letterSpacing: isAr ? "0" : "-0.01em" }}>{step.headline}</h3>
              <p style={{ fontFamily: hFont, fontSize: isAr ? "clamp(0.8rem,1.4vw,0.95rem)" : "clamp(0.75rem,1.3vw,0.9rem)", color: "var(--fg-dim)", fontWeight: isAr ? 400 : 300, lineHeight: isAr ? 1.8 : 1.6 }}>{step.body}</p>
              ...
              {step.timing && (
                <div style={{ marginTop: "0.6rem", display: "flex", alignItems: "center", gap: "0.4rem", flexDirection: isAr ? "row-reverse" : "row" }}>
                  ...
                  <span style={{ fontFamily: mFont, fontSize: isAr ? "0.7rem" : "0.55rem", color: "var(--accent-dim)", letterSpacing: isAr ? "0" : "0.1em" }}>{step.timing}</span>
                </div>
              )}
            </div>
          ))}
