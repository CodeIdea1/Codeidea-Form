...
export default function Journey({ tr, lang }: { tr: Translations; lang: Lang }) {
  ...
  return (
    <section style={{ width: "100%", height: "100%", minHeight: "100svh", display: "flex", flexDirection: "column", justifyContent: "center", padding: "clamp(4rem,8vw,6rem) clamp(1.5rem,8vw,7rem)", position: "relative", overflow: "hidden" }}>
      ...
      <h2 style={{ fontFamily: hFont, fontSize: "clamp(1.8rem,4.2vw,3.2rem)", fontWeight: 700, letterSpacing: isAr ? "-0.01em" : "-0.025em", lineHeight: isAr ? 1.3 : 1.1, color: "var(--fg)", marginBottom: "clamp(2rem,5vw,4rem)", maxWidth: "600px", textAlign: mirror ? "right" : "left", marginLeft: mirror ? "auto" : 0 }}>
      ...
      <div style={{ position: "relative" }}>
        {/* Connecting line */}
        <div className={`${s.connector} ${mirror ? s.connectorRtl : s.connectorLtr}`} />

        <div className={`${s.grid} ${mirror ? s.gridRtl : ""}`}>
          {tr.steps.map((step) => (
            <div key={step.num} className={s.step} style={{ position: "relative", paddingTop: "clamp(1.5rem,3vw,2.5rem)", textAlign: mirror ? "right" : "left" }}>
              {/* Node */}
              <div style={{ position: "absolute", top: 0, left: mirror ? "auto" : 0, right: mirror ? 0 : "auto", width: "clamp(0.9rem,1.5vw,1.2rem)", height: "clamp(0.9rem,1.5vw,1.2rem)", border: "1px solid var(--accent)", background: "var(--bg)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1 }}>
                <div style={{ width: "3px", height: "3px", background: "var(--accent)", borderRadius: "50%" }} />
              </div>

              <div style={{ display: "flex", alignItems: "baseline", gap: "0.5rem", marginBottom: "0.5rem", flexDirection: mirror ? "row-reverse" : "row" }}>
              ...
              {step.timing && (
                <div style={{ marginTop: "0.6rem", display: "flex", alignItems: "center", gap: "0.4rem", flexDirection: mirror ? "row-reverse" : "row" }}>
      ...
      <div style={{ position: "absolute", left: mirror ? "clamp(1.5rem,8vw,7rem)" : "auto", right: mirror ? "auto" : "clamp(1.5rem,8vw,7rem)", bottom: "clamp(2rem,6vw,5rem)", fontFamily: "var(--font-geist-mono)", fontSize: "clamp(6rem,18vw,16rem)", fontWeight: 700, color: "var(--ghost-num)", letterSpacing: "-0.05em", userSelect: "none", pointerEvents: "none", lineHeight: 1 }}>03</div>
    </section>
  );
}
